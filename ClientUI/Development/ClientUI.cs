...
namespace ClientUI
{
    ...
    public partial class AgvClientUI : Form
    {
        ...
        private void CarPosConfirm() {
            ...
            //if (similarity > 0.85) {
            mCarInfo.SetPosition(nowOdometry.x, nowOdometry.y, nowOdometry.theta * 180 / Math.PI);
            MapGL.SetCarPos(mCarInfo.ToCartesianPos());
            DrawLaser(mCarInfo);
            //}
        ...
        private List<MapLine> PathEncoder(string pack) {
            string[] pathArray = pack.Split(',');
            List<MapLine> rtnLine = new List<MapLine>();
            for (int i = 0; i < pathArray.Length - 5; i += 2) {
                rtnLine.Add(new MapLine(
                ...
                if (!mBypassSocket) {
                    ...
                    do {
                        ...
                        if (first == 1) {
                            ...
                            prog = new CtProgress("Get File", $"Get {fileName}");
                            /* Read file name */
                    ...
                    prog = new CtProgress("Get File", $"Get File");
                    SpinWait.SpinUntil(() => false, 1000);
                    fileName = "FileName";
                }
                Testing.UnLockOriOperator(true);
                SetBalloonTip($"Get {fileName}", $"The {fileName} is downloaded", ToolTipIcon.Info, 10);
        ...
        private void tsk_RecvPath(object obj) {
            ...
            try {
                ...
                if (strArray[0] == "Path" && !string.IsNullOrEmpty(strArray[1])) {
                    MapGL.DrawPath(PathEncoder(strArray[1]));
                }
                //else
        ...
        /// 車子資訊接收執行緒
        /// </summary>
        private void tsk_RecvCmd(object obj) {
            SocketMonitor soxMonitor = obj as SocketMonitor;
        ...
        private void DrawLaser(CarInfo info) {
            ...
            List<Point> ObstaclePoint = new List<Point>();
            int idx = 0;
        ...
        private void LoadMap(string mapPath) {
            ...
            MapGL.DrawMap(dispLines, dispPoint, goalList);
            ...
            if (!mBypassLoadFile) {//無BypassLoadFile
                ...
                if (dataLength != 0) {
                    for (int n = 0; n < dataLength; n++) {
                        ...
                        MapGL.DrawOri(carPos, laserData);
                        carPos = null;
                        laserData = null;
                    }
                }
