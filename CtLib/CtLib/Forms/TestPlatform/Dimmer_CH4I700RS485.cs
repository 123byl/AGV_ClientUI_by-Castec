...
namespace CtLib.Forms.TestPlatform {
	...
	public partial class Dimmer_CH4I700RS485 : Form {

		private CtDimmer_CH4I700RS485 mDimmer = new CtDimmer_CH4I700RS485();
		private Dictionary<Channels, NumericUpDown> mLightValues = new Dictionary<Channels, NumericUpDown>();

		/// <summary>建構調光器控制介面</summary>
		public Dimmer_CH4I700RS485() {
			InitializeComponent();
		...
		private void SetUIEnable(bool enabled) {
			...
			CtInvoke.ControlEnabled(gbAll, enabled);
		...
		private void btnConnect_Click(object sender, EventArgs e) {
			if (mDimmer.IsConnected) {
				...
				List<bool> locStt;
				if (mDimmer.Lock(out locStt)) {
                    CtInvoke.CheckBoxChecked(chkCh1Lock, locStt[0]);
                    CtInvoke.CheckBoxChecked(chkCh2Lock, locStt[1]);
                    CtInvoke.CheckBoxChecked(chkCh3Lock, locStt[2]);
                    CtInvoke.CheckBoxChecked(chkCh4Lock, locStt[3]);
                    CtInvoke.CheckBoxChecked(chkAllLock, locStt.All(val => val));
		...
		private void LockLights(object sender, EventArgs e) {
            ...
            bool stt = chk.Checked;
			Channels ch = (Channels)CtConvert.CByte(chk.Tag);
			if (mDimmer.Lock(ch, stt)) CtMsgBox.Show("鎖定", "鎖定 " + ch.ToString() + "成功！");
			else CtMsgBox.Show("鎖定", "鎖定 " + ch.ToString() + "失敗 :(", MsgBoxBtn.OK, MsgBoxStyle.Error);
		}

		private void btnSetAllAsSame_Click(object sender, EventArgs e) {
			if (mDimmer.SetLight((int)nudAll.Value)) CtMsgBox.Show("設定數值", "設定所有通道成功！");
			else CtMsgBox.Show("設定數值", "設定所有通道失敗 :(", MsgBoxBtn.OK, MsgBoxStyle.Error);
		}

		private void chkAllLock_CheckedChanged(object sender, EventArgs e) {
            bool stt = chkAllLock.Checked;
            if (mDimmer.LockAll(stt)) {
                CtInvoke.CheckBoxChecked(chkCh1Lock, stt);
                CtInvoke.CheckBoxChecked(chkCh2Lock, stt);
                CtInvoke.CheckBoxChecked(chkCh3Lock, stt);
                CtInvoke.CheckBoxChecked(chkCh4Lock, stt);
                CtMsgBox.Show("鎖定", "鎖定所有旋鈕成功！");
            } else CtMsgBox.Show("鎖定", "鎖定所有旋鈕失敗 :(", MsgBoxBtn.OK, MsgBoxStyle.Error);
		}
	}
}
