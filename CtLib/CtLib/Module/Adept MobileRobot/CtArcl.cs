...
namespace CtLib.Module.Adept {
    ...
    /// </summary>
    public class CtArcl : IDisposable {
        ...
        /// 
        /// </code></remarks>
        public static readonly CtVersion @Version = new CtVersion(1, 0, 0, "2015/03/12", "William Huang");
        ...
        public enum ArclEvents : byte {
            /// <summary>
            /// 與 ARCL 之連線狀態改變
            /// <para>回傳數值型態為 bool  (True)連接 (False)斷線</para>
            /// </summary>
            SOCKET_CONNECTION,
            ...
            /// <para>資料回傳型態為 bool  (True)觸發E-Stop  (False)安全無事</para>
            /// </summary>
            E_STOP
        }
        ...
        public bool TcpTransferMode { get; internal set; }
        ...
        /// </summary>
        /// <param name="tcpMode">是否為 TCP/IP 模式? (True)使用TCP/IP，以CtSyncSocket進行連線    (False)使用串列埠，以CtSerial連線</param>
        ...
        public CtArcl(CtSyncSocket socket) {
            TcpTransferMode = true;

            mSocket = socket;
            if (mSocket == null) mSocket = new CtSyncSocket();
        ...
        void mSocket_OnSocketEvents(object sender, CtSyncSocket.SocketEventArgs e) {
            if (e.Event == CtSyncSocket.SocketEvents.DATA_RECEIVED) {
                mQueue.Enqueue((e.Value as CtSyncSocket.DataInfo).Data.ToString());
            } else if (e.Event == CtSyncSocket.SocketEvents.CONNECTED_WITH_SERVER) {
                RaiseEvents(ArclEvents.SOCKET_CONNECTION, (e.Value as CtSyncSocket.ConnectInfo).Status);
            }
        }

        void mSerial_OnSerialEvents(object sender, CtSerial.SerialEventArgs e) {
            if (e.Event == CtSerial.SerialPortEvents.DATA_RECEIVED) {
            ...
            try {
                ...
                do {
                    try {
                        ...
                        Thread.Sleep(THREAD_SCAN_DELAY);
