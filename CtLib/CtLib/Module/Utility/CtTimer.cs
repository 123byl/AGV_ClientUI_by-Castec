...
namespace CtLib.Module.Utility {
	...
	public class CtTimer : ICtVersion {
		...
		///     
		/// 1.0.1  Ahern [2016/01/17]
		///		\ TimeSpan 如欲讀取等待時間，需使用 TotalMilliseconds
		///		+ 可使用 async / Task 等非同步方法的 WaitTimeoutAsync
		///		
		/// 1.0.2  Ahern [2016/07/15]
		///		- async / Task 非同步方法，於某些情況會 Thread Blocking，尚待釐清
		///     
		/// </code></remarks>
		public CtVersion Version { get { return new CtVersion(1, 0, 1, "2016/01/17", "Ahern Kuo"); } }
		...
		public class Timer : IDisposable {
			...
			/// <para>簽章必須符合 <see cref="Action{T}"/>，即不具回傳值，且含有單一 object 引數的方法</para>
			/// </param>
			/// <param name="repeat">是否持續觸發？  (<see langword="true"/>)固定間隔持續觸發  (<see langword="false"/>)僅觸發一次</param>
			/// <param name="autoStart">是否於此建構方法後啟動計時器？  (<see langword="true"/>)建構後立即啟動  (<see langword="false"/>)自行使用 <see cref="Start()"/> 啟動</param>
			...
			/// <returns>是否啟動成功？ (<see langword="true"/>)啟動成功  (<see langword="false"/>)無法變更時間間隔</returns>
			public bool Start() {
				bool result = false;
				if (mTimer != null) {
			...
			/// <returns>計時器是否成功停止？  (<see langword="true"/>)已停止  (<see langword="false"/>)無法將計時間隔設為無限</returns>
			public bool Stop() {
				bool result = false;
				if (mTimer != null) {
			...
			/// <param name="repeat">觸發後是否持續觸發？  (<see langword="true"/>)固定間隔持續觸發  (<see langword="false"/>)僅觸發一次</param>
			/// <returns>是否觸發成功？ (<see langword="true"/>)觸發成功  (<see langword="false"/>)無法變更時間間隔</returns>
		...
		public class TimeoutToken : IDisposable {
			...
			/// <summary>取得取消物件之 <see cref="CancellationToken"/></summary>
			public CancellationToken CancelToken { get { return mCncSrc.Token; } }
			#endregion
		...
		/// <returns>是否逾時?  (<see langword="true"/>)逾時  (<see langword="false"/>)於時間內完成工作</returns>
		/// <example><code language="C#">
		/// private void example() {
		...
		public static bool WaitTimeout(TimeSpan tmoTime, Action<TimeoutToken> action) {
			return WaitTimeout((int)tmoTime.TotalMilliseconds, action);
		}
		...
		public static bool WaitTimeout(TimeSpan tmoTime, Action<TimeoutToken, object[]> action, params object[] param) {
			return WaitTimeout((int)tmoTime.TotalMilliseconds, action, param);
		}
		...
		public static void WaitTimeoutEx(TimeSpan tmoTime, Action<TimeoutToken> action) {
			WaitTimeoutEx((int)tmoTime.TotalMilliseconds, action);
		}
		...
		public static void WaitTimeoutEx(TimeSpan tmoTime, Action<TimeoutToken, object[]> action, params object[] param) {
			WaitTimeoutEx((int)tmoTime.TotalMilliseconds, action, param);
		}
