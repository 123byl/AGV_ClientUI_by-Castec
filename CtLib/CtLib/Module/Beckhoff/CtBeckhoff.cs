...
namespace CtLib.Module.Beckhoff {
    ...
    /// </code></example>
    public class CtBeckhoff : IDisposable {
        ...
        ///     + 使用泛型之 GetValue
        /// </code></remarks>
        public static readonly CtVersion @Version = new CtVersion(1, 0, 4, "2015/03/02", "Ahern Kuo");
        ...
        public class SymbolEventArgs : EventArgs {
            ...
            public SymbolEventArgs(string strName, object objValue) {
                Name = strName;
                Value = objValue;
            }
        }
        ...
        /// <summary>Beckhoff PLC 內部資料型態</summary>
        ...
        public event EventHandler<SymbolEventArgs> OnSymbolChanged;
        ...
        protected virtual void OnSymbolEventChanged(SymbolEventArgs e) {
            EventHandler<SymbolEventArgs> handler = OnSymbolChanged;
            if (handler != null)
                handler(this, e);
        }
        ...
        public CtBeckhoff(bool enbMsg = true) {
        ...
        /// <param name="autoConnect">是否自動連線?  (True)完成設定後自動進行連線 (False)完成後不施行任何動作</param>
            ...
            try {
                ...
                do {
                    if (strID.Split(CtConst.CHR_PERIOD, StringSplitOptions.RemoveEmptyEntries).Length == 6) {
                        ...
                        CtInput frmInput = new CtInput();
                        if (frmInput.Start(CtInput.InputStyle.TEXT, "Beckhoff Ads NetID", "請輸入 Beckhoff PLC之NetID", out strID) != Stat.SUCCESS) {
                            stt = Stat.ER4_BKF;
                ...
                if (mAdsClnt.IsConnected) {
                    ...
                    OnBoolEventOccur(new BoolEventArgs(BoolEvents.Connection, true));   //如果連線成功，觸發Event
                } else
        ...
        private void ExceptionHandle(Stat stt, string title, string content) {
            ...
            if (EnableMessage) {
                ...
                /*-- 發報 --*/
                OnMessageUpdate(new MessageEventArgs(msgType, title, content));
            }
        ...
        private void ExceptionHandle(Stat stt, Exception ex) {
            ...
            if (EnableMessage) {
                ...
                /*-- 發報 --*/
                OnMessageUpdate(new MessageEventArgs(msgType, method, ex.Message));
            }
        }
        ...
        private object ReadSymbol(string symName) {
            object objTemp = null;
            /*-- 尋找並建立SymbolInfo --*/
            ITcAdsSymbol symInfo = mAdsClnt.ReadSymbolInfo(symName);

            /*-- 如有找到該變數則取得其值 --*/
            if (symInfo != null) {
                /*-- 確認是否為陣列，如是陣列或結構不應使用此Function --*/
                if ((symInfo.Datatype != AdsDatatypeId.ADST_BIGTYPE) && (symInfo.Datatype != AdsDatatypeId.ADST_MAXTYPES) && (!symInfo.Type.Contains("ARRAY")))
            ...
            if (symInfo != null) {
                switch (symInfo.Datatype) {
                    ...
                    default:
                        ...
                        throw (new Exception("變數" + varName + "為無法判別的" + Enum.GetName(typeof(AdsDatatypeId), symInfo.Datatype) + "資料型態"));
                }
            }
            ...
            do {
                switch (symInfo.Datatype) {
                    ...
                    default:
                        ...
                        if (symInfo.Type.ToUpper().Contains("TIME"))
                            lstTemp.Add(bnrRdr.ReadInt32());    //TIME使用Int32讀取
                        else
                            throw (new Exception("變數" + varName + "為無法判別的" + Enum.GetName(typeof(AdsDatatypeId), symInfo.Datatype) + "資料型態，請檢查該型態並Debug"));
        ...
        public void SetValue(string varName, object value) {
            ...
            switch (symInfo.Datatype) {
                ...
                case AdsDatatypeId.ADST_UINT8:
                    ...
                    break;
                default:
                    throw (new Exception("變數" + varName + "為目前不支援的" + Enum.GetName(typeof(AdsDatatypeId), symInfo.Datatype) + "資料型態"));
            }
        ...
        void mAdsClnt_AdsNotificationEx(object sender, AdsNotificationExEventArgs e) {
            OnSymbolEventChanged(new SymbolEventArgs(e.UserData.ToString(), e.Value));
        }
