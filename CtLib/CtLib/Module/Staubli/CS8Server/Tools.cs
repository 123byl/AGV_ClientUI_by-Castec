...
namespace Staubli.Robotics {
	internal class Tools {
		...
		public static void getRxRyRzCoord(Frame x_fr, out double x_Rx, out double x_Ry, out double x_Rz) {
			...
			if (l_sinRy < (-1.0 + SMALL_FLOAT * SMALL_FLOAT / 2.0)) {
				...
				if ((x_fr.az < 0.0) && (x_fr.nx < 0.0)
				   && ((Math.Abs(l_sinRy) > VERY_SMALL_FLOAT) || (Math.Abs(x_fr.ay) > VERY_SMALL_FLOAT))) { // if cosRy > 0, and both cosRx and cosRz are < 0, choose Ry so that
		...
		private static void getSinCos(double x_angle, out double x_sin, out double x_cos) {
			double l_t, l_t2;
